...
# teste em usar um filtro butterWorth passsa baixa ## não deu certo
ordem = 2
...
#plot_graph_curves(time_stamps,RoiGreen, RoiGray1, EqualizadeGreenRoi1,EqualizedGrayRoi1, outputImageGraph)
...
pcrt = PCRT.fromVideoFile(video_path, roi=roi1, displayVideo=False, exclusionMethod='best fit', exclusionCriteria=9999)
print(pcrt)
pcrt.showPCRTPlot()
outputFilePCRT = f"pCRTRoiGreen1{video_name}.png"
...
maxPicoPositivo, maxPicoNegativo = DerivadaDoSinal(EqualizedGrayRoi1,time_stamps)
#print(maxPicoPositivo)
#tempoMaxPicoPositivo = time_stamps[maxPicoPositivo]

maxPicoPositivo=maxPicoPositivo+4
time_stamps=time_stamps[maxPicoPositivo:]
RoiGray1 = EqualizedGrayRoi1[maxPicoPositivo:]

# Define a função exponencial generalizada
def exponencial_generalizada(x, k, a, b):
    
    """
    Calcula a função exponencial generalizada a * (1 + k * x)**(1/k) + b para um array x.
    Aqui, 'a' e 'b' são parâmetros adicionais para escala e deslocamento vertical.

    :param x: Array de valores de x para os quais a exponencial será calculada.
    :param k: Parâmetro que generaliza a exponencial.
    :param a: Fator de escala da exponencial.
    :param b: Deslocamento vertical da função.
    :return: Array contendo os valores da exponencial generalizada ajustada.
    """
    if k == 0:
        # Se k é zero, usamos a exponencial natural com o fator de escala 'a' e deslocamento 'b'
        return a * np.exp(x) + b
    else:
        # Caso contrário, usamos a fórmula generalizada com os parâmetros 'a' e 'b'
        return a * np.power(1 + k * x, 1 / k) + b



# Realiza o ajuste de curva para a função exponencial generalizada

p0 = (0.1, -1, 10)  
popt, pcov = curve_fit(exponencial_generalizada, time_stamps, RoiGray1, p0=p0,maxfev=5000)

# Parâmetros otimizados
k_opt, a_opt, b_opt = popt

# Imprime os parâmetros ajustados
print(f"Parâmetros ajustados: k = {k_opt}, a = {a_opt}, b = {b_opt}")

# Plota os dados originais e a curva ajustada
plt.scatter(time_stamps, RoiGray1, label='Grayscale Histogram Equalization')
plt.plot(time_stamps, exponencial_generalizada(time_stamps, k_opt, a_opt, b_opt), 'r-', label='Exponential Genaralized Fit')
plt.title(f"Exponential Generalized Fit: a={a_opt:.3f}, b={b_opt:.3f}, k={k_opt:.3f}")
plt.xlabel("Time",fontsize=12)
plt.ylabel("Intensity of Grayscale Histogram Equalization",fontsize=12)
plt.legend()
plt.grid(True)
nameSave=f"ExpGeneralizada{video_name}.png"
