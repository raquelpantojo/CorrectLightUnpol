...
#gamma = 1.53
def plot_graph_curves(best_a, best_b, best_gamma, RoiGreen, ROI1Corrigida, ROICorrigidaCompleta,outputImageGraph):
    
    plt.subplot(311)
    plt.plot(time_stamps, RoiGreen, label=f"Canal Verde ROI 1 ", color='darkgreen')
    plt.set_xlabel('Tempo (s)')
    plt.set_ylabel('Intensidade do Canal Verde')
    plt.legend()

    plt.subplot(312)
    plt.plot(time_stamps, ROI1Corrigida, label=f"Canal Verde corrigido", color='green')
    plt.set_title(r"$\gamma=$" + f"{best_gamma:.2f}")
    plt.set_xlabel('Tempo (s)')
    plt.set_ylabel('Intensidade do Canal Verde')
    plt.legend()
    ...
    plt.set_title(f"a={best_a:.2f}_b={best_b:.2f} $\gamma$={best_gamma:.2f}")
    plt.set_xlabel('Tempo (s)')
    plt.set_ylabel('Intensidade do Canal Verde')
    plt.legend()
    ...
    plt.tight_layout()
    outputImageGraph = f"PertoGrafico_a={best_a:.2f}_b={best_b:.2f}_g={best_gamma:.2f}_{folder_name}ROI{i+1}ROI{j+1}completo.png"
    plt.savefig(outputImageGraph, dpi=600)
    plt.show()
    plt.close()
...
for roi_pair in close_to_one_ratios.keys():
    ...
    for round in range(num_rounds):
        ...
        plot_graph_curves(best_a, best_b, best_gamma, RoiGreen, ROI1Corrigida, ROI1CorrigidaCompleto,outputImageGraph)
