...
RoiGreen1_selected=np.array(RoiGreen1_selected)
...
p0 = (0.1, 1, 100)  
popt, pcov = curve_fit(exponencial_generalizada, time_stamps, RoiGreen1_selected, p0=p0)
...
plt.scatter(time_stamps, RoiGreen1_selected, label='Dados originais')
plt.plot(time_stamps, exponencial_generalizada(time_stamps, k_opt, a_opt, b_opt), 'r-', label='Curva ajustada')
plt.title('Ajuste de Curva Exponencial Generalizada')
plt.xlabel('x')
plt.ylabel('y')
plt.legend()
plt.grid(True)
...
max_index = np.argmax(RoiGreen1_selected)
shift_frame = 0
max_index_shifted = min(max_index + shift_frame, len(RoiGreen1_selected))

timeStampsShiftFrame=time_stamps[max_index_shifted:]
ROICorrigidaShiftFrame = RoiGreen1_selected[max_index_shifted:]
