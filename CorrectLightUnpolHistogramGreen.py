...
def histogram_green(video_path, roi1, roi2, roi3, roi4, firstFrame, lastFrame, output_image_path):
    import cv2 as cv
    import numpy as np
    import matplotlib.pyplot as plt
    import sys
    ...
    # Extrai as ROIs para cada frame
    rois_10 = [
        frame_10[roi[1]:roi[1] + roi[3], roi[0]:roi[0] + roi[2], 1]  # Canal verde
        for roi in [roi1, roi2, roi3, roi4]
    ]
    rois_400 = [
        frame_400[roi[1]:roi[1] + roi[3], roi[0]:roi[0] + roi[2], 1]  # Canal verde
        for roi in [roi1, roi2, roi3, roi4]
    ]

    # Calcula os histogramas para as ROIs do canal verde
    hists_10 = [cv.calcHist([roi], [0], None, [256], [0, 256]) for roi in rois_10]
    hists_400 = [cv.calcHist([roi], [0], None, [256], [0, 256]) for roi in rois_400]

    # Plota os histogramas com sobreposição
    plt.figure(figsize=(12, 6))

    for i, (hist_10, hist_400) in enumerate(zip(hists_10, hists_400), start=1):
        plt.subplot(2, 2, i)
        plt.title(f'Histograma ROI {i} (Canal Verde)')
        plt.plot(hist_10, label=f'Frame {firstFrame}', color='green')
        plt.plot(hist_400, label=f'Frame {lastFrame}', color='lime')
        plt.xlabel('Intensidade')
        plt.ylabel('Frequência')
        plt.legend()
...
def histogram_roi_over_time(video_path, roi, maxPicoPositivo, num_frames, output_image_path):
    import cv2 as cv
    import numpy as np
    import matplotlib.pyplot as plt
    ...
    # Lista para armazenar histogramas
    histograms = []
    frame_indices = []

    # Processa frames deslocados de 10 em 10
    for i in range(num_frames):
        frame_index = maxPicoPositivo + i * 10
        cap.set(cv.CAP_PROP_POS_FRAMES, frame_index)
        ret, frame = cap.read()

        if not ret:
            print(f"Não foi possível ler o frame {frame_index}. Encerrando.")
            break

        # Extrai a ROI do canal verde
        roi_frame = frame[roi[1]:roi[1] + roi[3], roi[0]:roi[0] + roi[2], 1]

        # Calcula o histograma
        hist = cv.calcHist([roi_frame], [0], None, [256], [0, 256])
        histograms.append(hist)
        frame_indices.append(frame_index)

    cap.release()

    # Plota os histogramas ao longo do tempo
    plt.figure(figsize=(10, 6))
    for idx, hist in zip(frame_indices, histograms):
        plt.plot(hist, label=f'Frame {idx}')
    
    plt.title('Histograma do Canal Verde - ROI1')
    plt.xlabel('Intensidade')
    plt.ylabel('Frequência')
...
firstFrame = maxPicoNegativo
lastFrame = maxPicoPositivo # ponto de retirada do dedo 

###################### Histograna da imagem ####################
# função para calcular o histograma de uma imagem
#output_image_path = f"FigureHistogramGreen{firstFrame}{lastFrame}.png"
#histogram_green(video_path, roi1, roi2, roi3, roi4, firstFrame, lastFrame, output_image_path)

output_image_path = f"FigureHistogramGreen{firstFrame}{lastFrame}.png"
histogram_roi_over_time(video_path, roi1, lastFrame, 10, output_image_path)